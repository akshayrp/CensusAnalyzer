...
public class CensusAnalyser
{
   ...
   public int loadIndiaCensusData(String csvFilePath) throws CensusAnalyserException
   {
      ...
      try (Reader reader = Files.newBufferedReader(Paths.get(csvFilePath)))
      {
         ...
         StreamSupport.stream
               (csvIterable.spliterator(), false).forEach(censusCSV -> censusMap.put(censusCSV.state, new CensusDAO(censusCSV)));
   ...
   {
      Iterator<USCensusCSV> csvFileIterator;
      try (Reader reader = Files.newBufferedReader(Paths.get(csvFilePath)))
      {
         ICSVBuilder csvBuilder = CSVBuilderFactory.createCSVBuilder();
         try
         {
            csvFileIterator = csvBuilder.getCSVFileIterator(reader, USCensusCSV.class);
         }
         catch (RuntimeException e)
         {
            throw new CensusAnalyserException(e.getMessage(), CensusAnalyserException.ExceptionType.UNABLE_TO_PARSE);
